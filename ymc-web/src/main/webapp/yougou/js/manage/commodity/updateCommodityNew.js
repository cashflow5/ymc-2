...
goodsAdd.commodity.cat.makeReadonly = function() {
	//显示为只读版的商品分类
	//三个分类都选择了再设置为只读
	if($.trim($("#category1").val())!=""&&$.trim($("#category2").val())!=""&&$.trim($("#category3").val())!=""){
...
function propertiesTargetFun() {
	...
	for (var i = 0, len = commdoityProps.length; i < len; i++) {
		...
		if (valueType == '0') {
			...
			$("#goods_prop_radio_"+ propItemNo+"_"+propValueNo).attr('checked', true);
			...
			goodsAdd.properties.radioClick($("#goods_prop_radio_"+ propItemNo+"_"+propValueNo));
		} else if (valueType == '1') {
			//属性多选
...
goodsAdd.commodity.setProperties = function(commdoityProps) {
	...
	function propertiesCondition() {
		return $("#goods_prop_layer .detail_item_content input").length > 1;
	}
...
goodsAdd.commodity.setColor = function(colorNo, colorName) {
	...
	var radios = $("input[name='colorSelect']");
	var haveChecked = false;
	for(var i=0,_len=radios.length;i<_len;i++){
		if(radios.eq(i).val()==colorName){
			radios.eq(i).attr('checked', true);
			haveChecked = true;
		}
	}
	if(!haveChecked){
		var conDiv = $("<div class='color_con'></div>");
		$("<label colorName='"+colorName+"'><input name='colorSelect' type='radio' checked='checked' value='"+colorName+"'><span class='goods_color_layer_colorname'>"+colorName+"</span></label>").bind("click",function(){
			$(".color_input").hide();
			...
			input.blur(function(){
				...
				$("#goods_specName").val(input.val());
			});
			$("#goods_specName").val(input.val());
			$("#goods_product_color_td").text(input.val());
		}).appendTo(conDiv);
		$("<input type='text' class='color_input' value="+colorName+">").appendTo(conDiv);
		$("#colorClearDiv").before($(conDiv));
	}
};
...
function sizeTargetFun() {
	if(goodsAdd.commodity.originSize && goodsAdd.commodity.originSize.length > 0) {
		...
		for (var i = 0, len = products.length; i < len; i++) {
			...
			//按尺码设置价格
			if(isSetSizePrice){
				$("#goods_salePriceStr_" + prod.sizeNo).val(goodsAdd.commodity.sellPrice);
				$("#goods_publicPriceStr_"+ prod.sizeNo).val(goodsAdd.commodity.markPrice);
				$("#goods_salePriceStr_" + prod.sizeNo).attr("productId",prod.id);
				$("#goods_salePriceStr_" + prod.sizeNo).attr("productNo",prod.productNo);
				$("#goods_publicPriceStr_"+ prod.sizeNo).attr("productId",prod.id);
				$("#goods_publicPriceStr_" + prod.sizeNo).attr("productNo",prod.productNo);
			}
...
function showOrDisableBtn(op){
	...
	isforbiddenButton('commodity_save', opResult, function(){goodsAdd.submit.submitForm(false)});
	isforbiddenButton('commodity_audit', opResult, function(){goodsAdd.submit.submitForm(true)});
	isforbiddenButton('commodity_pre', opResult,function(){goodsAdd.submit.preview()});
	isforbiddenButton('commodity_save_f', opResult, function(){goodsAdd.submit.submitForm(false)});
	isforbiddenButton('commodity_audit_f', opResult, function(){goodsAdd.submit.submitForm(true)});
	isforbiddenButton('commodity_pre_f', opResult,function(){goodsAdd.submit.preview()});
}
