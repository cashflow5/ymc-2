            ...
            $(function() {
                ...
                $('#commodity_selling').blur(function() {
                    var value = $(this).val();
                    if (!isEmpty(value)) {
                        $('#commodity_selling_tip').addClass("none");
                    }
                });
                if(isSetSizePrice){
                	$(".isSizePrice").hide();
            ...
            goodsAdd.properties.load = function(propList) {
                ...
                for (var i = 0, len = propList.length; i < len; i++) {
                    ...
                    if (valueType == 1) {
                        ...
                        checkboxhtml += '<div class="shut_checkbox clearfix"><div class="shut_title">'+
        								...
        								'</div></div></div>';
                        html += formatString(
                            '<li class="goods_name_item"><div id="shutting-{#index}" class="clearfix shutting">' +
                            '	<div class="title fl"><em>{#must}</em>{#propItemName}：{#tips}</div><div class="shutting_span_val fl"></div>' +
                    	...
                    	var optionsHtml = '<select onchange="goodsAdd.properties.selectChange(this);" propitemno="'+propObj.propItemNo+'" '+
                    					  'name="'+propObj.propItemNo+'" propitemname="'+propObj.propItemName+'" show="1" style="margin: 0 5px 5px 25px; width: 188px;float:none ;"'+
                    					  ' id="'+propObj.propItemNo+'">';
                    	if(propObj.isShowMall != 0){
                        ...
                        optionsHtml+="</select>";
                        html += formatString(
                            '<li class="goods_name_item"><div class="goods_class clearfix">' +
                            '	<span class="title fl"><em>{#must}</em>{#propItemName}：{#tips}</span>' +
                            '	<span class="goods_span_val fl"></span>' +
                ...
                for (var i = 0, len = $propSelects.length; i < len; i++) {
                    ...
                    goodsAdd.properties.selectChange($(propSelect));
                }
            };
            ...
            function goods_submit(isSaveSubmit) {
                ...
                $.post(basePath+"/commodity/checkSensitiveWord.sc",{
                	...
                	styleNo:$("#commodity_style").val(),
                	supplierCode:$("#commodity_code").val(),
                	sellingPoint:$("#commodity_selling").val(),
                	prodDesc:goodsAdd.prodSpec.editor.text()
                	},function(json){
                	var nameArr = new Array(),wordArr = new Array(),idArr = new Array();
                	var sensiveWord = "";
                	$("input[name='followOperate']").remove();
                	if($(json.result).size()>0){
                		...
                		ygdg.dialog({
                            id: "submitDialog"
                        }).title('检测到敏感词提醒').content("<p class='cred'><b>检测到敏感词，建议处理后再继续</b></p>"+nameArr.join("，")+
                            		"检测到敏感词：<span class='cred'>"+$.unique(sensiveWord.split(",").sort()).join("，")+"</span>")
                            		.size('450px',null).position('50%','40%').button({
                                            name: '取消',
                                            callback:function(){
                                            	$.each(idArr,function(i,n){
                                            		$("#"+n+"_tip").removeClass("none").html("敏感词："+wordArr[i]);
                                            	});
                                            	$("#"+idArr[0]).focus();
                                            	$.post(basePath+"/commodity/logSensitiveWord.sc",{
                                            		content:$("#commodity_name").val()+";"+$("#commodity_code").val()+";"+goodsAdd.prodSpec.editor.text(),
                                                	styleNo:$("#commodity_style").val(),
                                                	supplierCode:$("#commodity_code").val(),
                                                	followOperate:0,
                                                	commodityNo:requestCommodityNo,
                                                	sensitiveWord:$.unique(sensiveWord.split(",").sort()).join()
                                            	});
                                            },
                                            focus: true
                                        },{
                                            name: '确认继续',
                                            callback: function () {
                                            	$("#secondForm").append('<input type="hidden" name="followOperate" value="1"/>');
                                            	submitForm(isSaveSubmit);
                                            	return false;
                                            }
                            		});
                	}else{
            ...
            function submitForm(isSaveSubmit){
            	ygdg.dialog({
                        id: "submitDialog"
                }).title("操作提示")
                .content('<img src="' + goodsAdd.url.loadingImgUrl + '" width="16" height="16" /> 正在提交,请稍候...')
                .size('200px',null).position('50%','40%').button({
                    name: '取消',
                    disabled: true
                },{name:'确认继续',disabled:true}).show();
            ...
            goodsAdd.validate.joinErrorList = function(){
            	var errorList = goodsAdd.validate.errorList;
            	var errorHtml = "";
                var error = null;
                for (var int = 0; int < errorList.length; int++) {
                    error = errorList[int];
                    errorHtml = errorHtml+(int+1)+"、"+error.errMsg+"<br/>";
                }
                return errorHtml;
            };
            ...
            goodsAdd.validate.validateThirdPartyCode = function() {
                ...
                var containsChinese = "第 {#num} 行的货品条码只能为非汉字的字符";
                ...
                for (var i = 0, len = $thirdPartyCodes.length; i < len; i++) {
                    ...
                    if (goodsAdd.validate.containsChinese("sku_"+sizeValue, thisValue, formatString(containsChinese, {
                            num: (i + 1)
                        }))) continue;
                }
                //校验货品条码是否重复
                                ...
                                for (var j = 0, _sublen = propVals.length; j < _sublen; j++) {
                                    ...
                                    if (checkBox.length > 0) { //存在多选
                                        if (!checkBox.attr("checked")) {
                                            ...
                                            $("#goods_prop_valuetype_"+item.propItemNo).siblings(".shutting_span_val").append(item.propValueName.split(";")[j]+",");
            ...
            goodsAdd.setNoChekckValue = function(){
            	...
            	var selectValueTypeObjs = $("input[id^='goods_prop_valuetype_'][value='0']");
            	$.each(selectValueTypeObjs,function(i,n){
            		var htmlStr =$(n).siblings(".goods_span_val").html();
            		if(!(htmlStr)){
            ...
            function deleteLastComma(){
            	var valObjs = $("[id^='goods_prop_valuetype']").siblings(".shutting_span_val");
            	var reg=/,$/gi;
            	$.each(valObjs,function(i,n){
            		$(n).text($(n).text().replace(reg,"")); 
            	});
            }
