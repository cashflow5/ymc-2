...
$(function() {
        //left的,编辑的hover
        $(".goods_name li").live("mouseover",function(){
        	var siblingselect=$(this).children(".goods_class").siblings().children("select");
        ...
        $('.btn_yes').live('click', function(event) {
            ...
            str = arr.join(",");
            if (!str) {
                $(this).parents('.shutting').parent().removeClass('select');
        ...
        function disabledOther(){
        	$('.shutting').parent().removeClass('select');
        ...
        $('.shutting_click').live('click', function(event) {
            ...
            curr_index=$('.shutting_click').index(this);
	        $('.shutting_click').each(function(index){
	            if(index!=curr_index){
	                if($('.shutting_val').eq(index).hasClass('show')){
	                  $('.shutting_click').eq(index).addClass('show').removeClass('hide'); 
	                }
	            }
	        });
        	disabledOther();
        ...
        $('.color_radio').live('click', function() {
            ...
            $('.size-list :checked').each(function(i, item) {
                ...
                strcontent += '<td data-id="' + item.value + '">' + 
                			...
                			'<td><input id="sku_' + item.value + '" maxlength="32" name="thirdPartyCode" class="goods_thirdPartyCode_txt" sizeValue="'+item.value+'" type="text" /></td><td>'+
        ...
        $('.size-list input[name="goods_sizeNo"]').live('click', function() {
            ...
            if ($this.prop('checked')) {
            	...
            	if(requestCommodityNo && isSetSizePrice){
            		...
            		if(idstr){
                		...
                		var sellStatus = $("#goods_sizeNo_display_" + preSizeNo).attr("sellStatus");
                		if(sellStatus=='2'){
                			ygdg.dialog.alert("该尺码对应的货品已经有销售数据,不允许删除!");
                			$("#goods_sizeNo_display_" + preSizeNo).attr("checked", "true");
                			return false;
                		}
                		//保存删除前的设置
            ...
            showtable(size_body);
            $("#goods_sizeNo_tip").addClass("none");
        });
...
$('[name=setting]').live('click', function(e) {
    if (e.target.id == 'rdenactmen') {
    	...
    	$('#enactmen').val("");
        $('#enactmen').attr('disabled', true);
        $("#goods_setting_tip").html("").addClass("none");
    }
});

//li的高度控制，2015-06-12
function li_height(){
    var goods_select_val=$(".goods_properties_select");
    goods_select_val.each(function(index){
        if(goods_select_val.eq(index).attr('show')){
            goods_select_val.eq(index).closest("li").addClass('h_44');
        }
    });
}
