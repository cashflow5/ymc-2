...
/**加载商品url*/
goodsAdd.url.getCommodityByNo = basePath + "/taobao/loadExtendById.sc?extendId=" + requestCommodityNo; 
...
goodsAdd.properties.collectPropIdInfo = function() {
	...
	//alert("goodsAdd.properties.collectPropIdInfo:"+$propItemNoTxts.legnth);
	...
	for (var i = 0, len = commdoityProps.length; i < len; i++) {
		...
		if (valueType == '0') {
			...
			if($("#shut" + propItemNo + "_" + propValueNo).length>0){
				...
				$("#goods_prop_valuetype_"+propItemNo).siblings(".shutting_span_val").append(propValueName+"<br/>");
...
//	}
	...
	for (var i = 0, len = ADD_OR_UPDATE_COMMODITY_IMAGE_COUNT; i < len; i++) {
		...
		var re=/.jpg$/;
		//先尝试从缓存取图片
		if(imageMessage.length>0&&i<imageMessage.length){

			var imgUrl = imageMessage[i].yougouUrl;//imageMessage[i].picUrl;
