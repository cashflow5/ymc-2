...
taobaoYougouItemCatPro.initData = function(){
	...
	$.ajax({
		...
		url : taobaoYougouItemCatPro.basePath+ "/taobao/getYougouTaobaoProTemplet.sc",
...
function ProRow(data,taobaoProList){
	...
	if(data.isShowMall==0){//显示是否是必填属性
		$("<td width='30%'>" +
				"<span class='yougouPropItemName' style='font-weight:bold'>"+data.propItemName+"<span>" +
	...
	var cancelBtn  =$("<a href='javascript:void(0)' class='closeIcon'><img src='"+taobaoYougouItemCatPro.basePath+"/yougou/images/taobao_pro_close.png'></a>").appendTo(endTd).bind("click",this.cancel);
}
ProRow.prototype =  {
...
function loadProVals(curObj){
	...
	$.ajax({
		...
		url : taobaoYougouItemCatPro.basePath+ "/taobao/getTaobaoItemProVal.sc",
		success : function(data) {
			if(data.resultCode == "200"){
				...
				for(var p=0,length = proValSelects.length;p<length;p++){
					...
					for(var v=0,length_v=proVals.length;v<length_v;v++){
						...
						if(proVal.yougouPropValueNo!=""&&proVal.yougouPropValueNo==vSelect.attr("yougouprovalue")){//已经绑定的属性值
							...
							if(vSelect.attr("yougouproname")==proVal.vName){
								$("<option selected='selected' value='"+proVal.vid+"'>"+proVal.vName+"</option>").appendTo(vSelect);
							}else{
...
/*	for(var i=0,length=youtouMustProps.length;i<length;i++){
	...
	if(errorList.length>0){
		...
		for(var i=0,len =errorList.length;i<len;i++ ){
			error = errorList[i];
			$("<tr><td style='padding:2px'><img src='/yougou/images/error.gif' class='goods_error_image'>"+error+"</td></tr>").appendTo(table);
		}
		$("#msgdiv").show();
		return;
	}
	...
	$.ajax({
		...
		url : taobaoYougouItemCatPro.basePath+ "/taobao/bindTaobaoYougouItemProAndVal.sc?catId="+taobaoYougouItemCatPro.catId+"&taobaoCid="+taobaoYougouItemCatPro.taobaoCid+"&catBandId="+taobaoYougouItemCatPro.bindId,
		success : function(data) {
			ymc_common.loading();
			if(data.resultCode == "200"){
				document.location.href=taobaoYougouItemCatPro.basePath+ "/taobao/bindTaobaoYougouItemProAndValSuccess.sc";
			}else{
				ygdg.dialog.alert(data.msg);
			}
		}
