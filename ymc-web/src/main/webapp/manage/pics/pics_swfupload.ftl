...
<script type="text/javascript" src="${BasePath}/yougou/js/ztree/jquery.ztree.excheck-3.5.min.js"></script>
	...
	<#if treeModes??>
		...
		$(document).ready(function(){
			...
			var n = zTree.getNodesByParam("id","${catalogId!''}");
			if(n){
			    zTree.selectNode(n[0],false);
			    onClick(null,zTree.setting.treeId,n[0]);
			}
		});
		//-->
	</SCRIPT>
...
<body>
	<div style="padding: 10px;">
		<div style="margin: 0 0 0px 0;padding: 10px 0px;">
			...
			<span style="display: inline-block;position: relative;"><a class="button btn1">
				<span><div id="spanButtonPlaceHolder">控件加载中...</div></span>
			</a>
			<a class="button btn2" onclick="javascript:$('#fsUploadProgress').find('a').click();">
				<span>清空列表</span>
			</a></span><input id="catalogId" type="hidden" value=""/><input id="shopId" type="hidden" value=""/>
			<span style="display: inline-block; margin-left: 189px;"><div id="catalogPath"></div></span>
		</div>
...
<script type="text/javascript">
var swfu;
var swfu_post_params = { merchantCode: "${merchantUsers.supplier_code!''}", replacement: "",JSESSIONID:"${jsessionId!''}"};
$(document).ready(function(){
	swfu = new SWFUpload({
