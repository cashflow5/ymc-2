...
<div class="normal_box">
		...
		<h1 class="modify-title">修改登录密码</h1>
                    <ul class="step-list clearfix">
                        ...
                        <li class="histoy">2 <p>修改登录密码</p></li>
                        <li class="curr complete"><img src="${BasePath}/yougou/images/modity-step-ok.png" alt="" /><p>完成</p></li>
                    </ul>
                    <div class="form-box">
                    <div class="<#if isScuess?? && isScuess==true>fdpwd_suc<#else>fdpwd_fail</#if>">
				    	 <div class="con" style="padding: 100px 0 0 90px">
				       	  <h2>${message}</h2>
				          <p class="mt20">您可立即返回 <a href="${BasePath}/merchants/security/updatePwd.sc" class="cblue">商家中心修改登录密码页面</a></p>
				          <p class="c9 mt5"><span id="time">5</span>秒后，页面自动跳转到商家中心修改登录密码页面</p>
				         </div>
    				</div>
    				</div>
    </div>
	 ...
	 <script type="text/javascript">
	$(function(){
		delayURL(basePath+"/merchants/security/updatePwd.sc");
	});
</script>
</body>
</html>