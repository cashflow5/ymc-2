...
<#if pageFinder?? && pageFinder.data?? && (pageFinder.rowCount > 0)>
	<div class="page">
		...
		<#if (pageFinder.pageCount > 1) >
			...
			<input id="query.page" name="query.page" value="${pageFinder.pageNo}"  onkeyup="value=value.replace(/[^\d]/g,'')"  class="inputtxt" type="text" value="1" title="输入页码后点击转到" style="float:left;height:20px;width:40px;text-align:center;" />
