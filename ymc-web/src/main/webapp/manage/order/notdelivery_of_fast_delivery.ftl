...
<body>
		...
		<div class="normal_box">
			...
			<p style="position:absolute;top:0;right:0;"><span class="fl icon_info"></span><span class="fl ml5" style="color:#999;">默认显示30天的订单，可设置条件查询更多订单，缺货订单请不要发货</span></p>
				<ul class="tab">
					...
					<li onclick="javascript:location.href='${BasePath}/order/fastdelivery/notdelivery.sc';" class="curr"><span>已导出未发货</span></li>
				...
				<div class="tab_content"> 
					...
					<div class="search_box">
						<form action="${BasePath}/order/fastdelivery/notdelivery.sc" id="queryVoform" name="queryVoform" method="post">
							<p>
								<span>
								...
								<span>
									<label style="width:170px;">商家款色编码：</label>
								...
								<span>
									<label style="width:170px;">商家货品条码：</label>
							...
							<p>
								...
								<span>
									<label style="width:106px;">导出时间：</label>
								...
								<span>
									<label style="width:100px;"><input name="singleCargo" type="checkbox" value="1" <#if (orderSubExpand.singleCargo)?? && orderSubExpand.singleCargo == '1'>checked="checked"</#if> />&nbsp;一单一货</label>
									<label title="勾选“合并发货”列表中筛选出收货人姓名、地址、手机等信息一致的订单，可按照实际需要对这些订单进行合单发货操作。"><input name="mergerCargo" type="checkbox" value="1" <#if (orderSubExpand.mergerCargo)?? && orderSubExpand.mergerCargo == '1'>checked="checked"</#if>  />&nbsp;合并发货</label>
								</span>
							</p>
							<p>
								<span>
					...
					<table class="list_table">
						<thead>
							<tr>
								<th width="5%">&nbsp;<input type="checkbox" id="checkboxall" onclick="javascript:$('#tbody :checkbox').attr('checked', this.checked);"/></th>
								<th width="8%">下单时间</th>
								<th width="12%">订单号</th>
								<th width="5%">订单金额</th>
								<th width="5%">收货人姓名</th>
							...
							<#if (pageFinder.data)?? && pageFinder.data?size != 0>
								...
								<#if orderintercept[item.orderSubNo]>
									...
									<tr>
										...
										<td style="width:60px;"><#if (item.exportedDate)??>${(item.exportedDate)?string('yyyy-MM-dd HH:mm:ss')}</#if></td>
										<td>
											购物清单<b>${(item.printShoppinglistCount)?default(0)}</b>次<br />
											快递单<b>${(item.printLogisticslistCount)?default(0)}</b>次
										</td>
										...
										<td style="width:140px;">
											...
											<a href="javascript:;" onclick="javascript:order.print.updateOutOfStockConfirm('${item.orderSubNo!''}','${item.orderSubId!''}');">缺货</a>&nbsp;|&nbsp;
	...
	<script type="text/javascript">
		...
		function doExportOrderdetail() {
			...
			$("#queryVoform").attr("action","${BasePath}/order/fastdelivery/doExportOrderdetail.sc?orderSubNos="+orderSubNos + "&flag=2");
			$("#queryVoform").submit();
			$("#queryVoform").attr("action","${BasePath}/order/fastdelivery/notdelivery.sc");
			var pathname = window.location.pathname;
			//重载当前页面
