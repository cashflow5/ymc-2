...
<title>优购商家中心-单据打印（已打印未发货）</title>
		...
		<div class="normal_box">
			...
			<p style="position:absolute;top:0;right:0;"><span class="fl icon_info"></span><span class="fl ml5" style="color:#999;">默认显示30天的订单，可设置条件查询更多订单，缺货订单请不要发货</span></p>
			    ...
			    <ul class="tab">
					...
					<li onclick="location.href='${BasePath}/order/toPrintNewNotDelivery.sc'" class="curr" ><span>已打印未发货</span></li>
					...
					<li onclick="location.href='${BasePath}/order/toPrintNewAll.sc'" > <span>全部</span></li>
				</ul>
				<div class="tab_content"> 
					<!--搜索start-->
					<div class="search_box">
						<form action="toPrintNewNotDelivery.sc" id="queryVoform" name="queryVoform" method="post">
							<!--是否一单一货-->
							...
							<p>
								...
								<span>
									<label><input id="chk" type="checkbox" <#if orderSubExpand.singleCargo??&&orderSubExpand.singleCargo=="1">checked</#if> />&nbsp;一单一货</label>
								</span>
								<span>
									...
									<td colspan="17" style="padding:0;text-align:left;">
										<div class="tb_dobox">
											<div class="dobox">
												...
												<a href="javascript:;" onclick="order.print.doExportOrder('/order/toPrintNewNotDelivery.sc')">导出拣货清单</a> 
							...
							<#if pageFinder??&&pageFinder.data??>
								...
								<#if orderintercept[item.orderSubNo]>
									...
									<tr>
										...
										<td><a href="javascript:;" onclick="javascript:order.print.toDetail('${item.orderSubNo!''}','MENU_DJDYX')">${item.orderSubNo!''}</a><#if item.originalOrderNo??&&item.originalOrderNo!=''><#if item.orderSubNo?substring(item.orderSubNo?index_of("_")+1)?number gt 200>[补]<#else>[换]</#if></#if></td>
										...
										<td style="text-align:left;">
											...
											<a href="javascript:ordernew.printSingle('${item.orderSubNo!''}')">补打购物清单</a> |<br /> 
