...
<body>
		...
		<div class="normal_box">
			...
			<p style="position:absolute;top:0;right:0;"><span class="fl icon_info"></span><span class="fl ml5" style="color:#999;">默认显示30天的订单，可设置条件查询更多订单，缺货订单请不要发货</span></p>
			   <ul class="tab">
					...
					<li onclick="location.href='${BasePath}/order/toPrintAll.sc'"  class="curr" > <span>全部</span></li>
				</ul>
				<div class="tab_content"> 
					<!--搜索start-->
					<div class="search_box">
						<form action="toPrintAll.sc" id="queryVoform" name="queryVoform" method="post">
							<!--是否一单一货-->
							...
							<!-- 当前页面的订单编号缓存 -->
							<input type="hidden" id="orderNOs" name="orderNOs" value="${orderNOs!''}" style="display: none;" />
							...
							<#if pageFinder??&&pageFinder.data??>
								<#list pageFinder.data as item>
									...
									<tr>
										...
										<td><#if (item.isException??)&&item.isException!=0>[异常]</#if><a href="javascript:;" onclick="javascript:order.print.toDetail('${item.orderSubNo!''}','MENU_DJDY')">${item.orderSubNo!''}</a><#if item.originalOrderNo??&&item.originalOrderNo!=''><#if item.orderSubNo?substring(item.orderSubNo?index_of("_")+1)?number gt 200>[补]<#else>[换]</#if></#if></td>
	...
	<script type="text/javascript">
		...
		function mysubmit(){
			//验证是否至少输入一个条件
			//if($("#orderSubNo").val()==""&&$("#originalOrderNo").val()==""&&$("#startTime").val()==""&&$("#endTime").val()==""&&$("#logisticsName").val()==""&&$("#suppliersColorModelsCode").val()==""&&$("#thirdPartyCode").val()==""&&$("#expressOrderId").val()==""&&$("#timeStartShipTime").val()==""&&$("#timeEndShipTime").val()==""&&$("#baseStatus").val()==""){
				//alert("请至少输入一项搜索条件");
				//return;
			//}
			if(!order.checkDate()){
