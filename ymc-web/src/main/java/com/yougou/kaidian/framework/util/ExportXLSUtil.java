...
public class ExportXLSUtil {
	...
	public static boolean exportExcel(String title, String[] headers, Collection<Object[]> dataset, String[] heddenAndUpdateColumns, Boolean[] amounts,
	...
	public static synchronized void exportEexcel(HttpServletResponse response, HttpServletRequest request, String title, String[] headers, Collection<Object[]> datas,
			String[] heddenAndUpdateColumns, Boolean[] amounts) throws Exception {
		OutputStream out = response.getOutputStream();
		FileInputStream is = null;
		String fileName = ""+new Date().getTime();
		String filePath = request.getSession().getServletContext().getRealPath(File.separator) + fileName + ".xls";
		OutputStream outOfGenerator = new FileOutputStream(filePath);
		ExportXLSUtil.exportExcel(fileName, headers, datas, heddenAndUpdateColumns, amounts, outOfGenerator, "yyyy-MM-dd");
		try {
			response.setContentType("application/vnd.ms-excel;charset=UTF-8");
			response.setStatus(response.SC_OK);
			if (title != null && !"".equals(title)) {
				Integer viewType = SessionUtil.getBrowingType(request);
				if (1 == viewType) {
			...
			int bytesRead = 0;
			while ((bytesRead = is.read(buffer, 0, (int) file.length())) != -1) {
			...
			File file = new File(filePath);
			if(file.exists()) {
				logger.error("销售明细是否删除："+(file.delete()?"是":"否"));
			}
		}
	}
}
