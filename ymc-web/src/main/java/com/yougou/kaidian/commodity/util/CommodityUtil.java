...
public class CommodityUtil {
	...
	private static final Logger logger = LoggerFactory.getLogger(CommodityUtil.class);
	 ...
	 * @return 
	 */
	public static List<CommodityVO> buildCommodityVOList(CommoditySubmitVo submitVo, 
		...
		for (int i = 0, len = submitVo.getSizeNo().length; i < len; i++) {
			...
			//分类id(必填)
			commodityVO.setCatId(getCatIdByThreeCategory(submitVo.getThreeCategory()));
			//分类编号(必填)
			commodityVO.setCatNo(getCatNoByThreeCategory(submitVo.getThreeCategory()));
			//分类结构名(必填)
			commodityVO.setCatStructname(getStructNameByThreeCategory(submitVo.getThreeCategory()));
			//分类名称(必填)
			commodityVO.setCatName(getCatNameByThreeCategory(submitVo.getThreeCategory()));
			//品牌编号(必填)
	 ...
	 * @return 
	 */
	public static CommodityVO buildCommodityVO(CommoditySubmitVo submitVo) {
		...
		//commodityVO.setOrderDistributionSide(submitVo.getOrderDistributionSide());
		// 买点
		commodityVO.setSellingPoint(StringUtils.chomp(submitVo.getSellingPoint().replace("\"", "")
				.replace("\n", "").replace("\\", "")
				.replaceAll("　", "")
					.replaceAll("\\s+", " ")//中间多个英文空格，保留一个
				.trim()));
	 ...
	 */
	public static com.yougou.pc.model.commodityinfo.Commodity buildCommodityUpdateVo(CommoditySubmitVo submitVo, 
		...
		//商品名称
		commodity.setCommodityName(StringUtils.chomp(submitVo.getCommodityName().replace("\"", "")
				.replace("\n", "").replace("\\", "")
				.replaceAll("　", "")
					.replaceAll("\\s+", " ")//中间多个英文空格，保留一个
				.trim()));
		...
		//分类编号 
		commodity.setCatNo(CommodityUtil.getCatNoByThreeCategory(submitVo.getThreeCategory()));
		//分类名称 
		commodity.setCatName(getCatNameByThreeCategory(submitVo.getThreeCategory()));
		//分类结构名称 
		commodity.setCatStructName(getStructNameByThreeCategory(submitVo.getThreeCategory()));
		//分类id
//		commodity.setcatId(getCatIdByThreeCategory(submitVo.getThreeCategory()));
		...
		commodity.setSellingPoint(StringUtils.chomp(submitVo.getSellingPoint().replace("\"", "")
				.replace("\n", "").replace("\\", "")
				.replaceAll("　", "")
					.replaceAll("\\s+", " ")//中间多个英文空格，保留一个
				.trim()));
			...
			for (int i = 0, len = submitVo.getPropItemNo().length; i < len; i++) {
				...
				for (int j = 0; j < propValueNos.length; j++) {
					if (StringUtils.isNotBlank(propValueNos[j])) {
						...
						if (StringUtils.isNotBlank(propItemId) && propValueNos.length == 1) {
						 ...
						 * 通过属性值匹配获取propItemId
						 */
						String _propItemId = getValueFormMapListByAnotherKey(
								submitVo.getPropIdList(), "propItemId", "propValueNo", propValueNos[j]);
						if (StringUtils.isNotBlank(_propItemId) && propValueNos.length > 1) {
							commodityProp.setId(_propItemId);
							commodityProp.setCommodityId(submitVo.getCommodityId());
						}
						//属性项编号
	 ...
	 * @return 成功则返回true
	 */
	public static boolean initMerchantUserInfoForAddOrUpdate(HttpServletRequest request, 
