...
<#if pageFinder?? && pageFinder.data?? && (pageFinder.rowCount > 0)>
	<div class="page">
		<#if (pageFinder.pageCount > 1) >
			...
			<#else>
				<a href="/tuan/prev${pageFinder.pageNo-1}.html" class="page-pre"  title="上一页" >上一页</a>
			</#if>
			...
			<#if (pageFinder.pageCount < 10) >
				<#list 1..(pageFinder.pageCount) as row>
					...
					<#if pageFinder.pageNo == row >
						<!-- 选中时的样式 -->
						<a href="/tuan/prev${row}.html" class="current" >${row}</a>
					<#else>
						<a href="/tuan/prev${row}.html" >${row}</a>
					</#if>
				</#list>
			...
			<#elseif ((pageFinder.pageCount - pageFinder.pageNo) < 5) >
				<#list 1..9 as row>
					<#if ((pageFinder.pageCount - pageFinder.pageNo) == (9- row)) >
						<!-- 选中时的样式 -->
						<a href="/tuan/prev${pageFinder.pageCount - 9 + row}.html" class="current"> ${pageFinder.pageCount  - 9 + row}</a>
					<#else>
						<a href="/tuan/prev${pageFinder.pageCount - 9 + row}.html"> ${pageFinder.pageCount  - 9 + row}</a>
						<!-- 默认的样式 -->
					</#if>
				</#list>
			...
			<#else>
				<#list 1..10 as row>
					...
					<a>	...</a>
					<#elseif (row == 9) >
						<a href="/tuan/prev${pageFinder.pageCount -1 }.html"  > ${pageFinder.pageCount -1}</a>
					<#elseif (row == 10) >
						<a href="/tuan/prev${pageFinder.pageCount}.html" > ${pageFinder.pageCount}</a>
					<#else>
						...
						<#else>
							...
							<#if (row == 4) >
								<!-- 选中时的样式 -->
								<a href="/tuan/prev${pageFinder.pageNo-4+row}.html" class="current" > ${pageFinder.pageNo-4+row}</a>
							<#else>
								<!-- 默认的样式 -->
								<a href="/tuan/prev${pageFinder.pageNo-4+row}.html"> ${pageFinder.pageNo-4+row}</a>
							</#if>
			...
			<#else>
				<a href="/tuan/prev${pageFinder.pageNo+1}.html" class="page-next"  title="下一页"  >下一页</a>
			</#if>
		</#if>
	</div>
</#if>
