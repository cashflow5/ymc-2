...
function deleteConfrimOrder(){
	if(!confirm("您确定要删除此召回返回申请单吗？")){	
			return false;
  	}
	viewReturnSpFrom.action = "${BasePath}/supply/manage/recallreturn/deleteSpOrder.sc";
	viewReturnSpFrom.submit();
}	
	...
	for(var i=0;i<cks.length;i++){   
		...
		success : function(resultMsg) {
			if (resultMsg) {
				...
				window.location.href="${BasePath}/supply/manage/recallreturn/toSaveSp.sc?mainId="+$("#returnDefectSpId").val()+"";
			} else {
				alert("提示：["+resultMsg+"]");
			}
		}
...
function toAddDetail() {
	...
	var toAddDetailUrl="${BasePath}/wms/stocksmanager/purchasereturn/toPurchaseReturnSel.sc?supplierCode="+supplierCode+"&supplierName="+supplierName+"&returnDefectConfirmId="+$('#returnDefectSpId').val()+"&queryTypeFlag=2&url=supply/purchase/recallreturn/common_recall_return_forspredef";
...
<div class="container">
	...
	<div class="list_content"> 
		...
		<div class="top clearfix">
			<ul class="tab">
				<li class="curr"><span>召回返回申请单提交</span> </li>
			</ul>
		</div>
		<!--当前位置end-->
		<div class="modify">
				 ...
				 <p>
					 <span>
				      ...
				      <span> 
						...
						<input type="hidden" id="goodsType" name ="returnDefectSp.goodsType"  value="${returnDefectSp.goodsType?if_exists}"/> 
				 	 </span>
				</p>
				<p>
   ...
   if(warehouseId == null || warehouseId =='' || warehouseName == null || warehouseName =='' || warehouseId == 'ALL'){
	...
	if(upTable.rows.length==1){
     ...
     if(!checkInput()){
		...
		$.ajax({ type : 'post', url: '${BasePath}/supply/manage/recallreturn/toUpdateReturnDefeSpDetail.sc', dataType: 'json', data: $('#mainFormUp').serialize(),
