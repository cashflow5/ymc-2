...
<script type="text/javascript">
	...
	function toMainMenue(){
		window.location.href="${BasePath}/supply/manage/returnDefect/toQueryReturnDefectConfirm.sc";
	}
...
$(document).ready(function(){					   
   ...
   function doSave(type) {
	   ...
	   if(type==2){
		...
		var bool=true;
	  //重组索引
	    //for(i=0;i<document.getElementById('subTable').rows.length-1;i++){
        //    var j=i+1;
		//	  var sysQty =document.getElementById("allocationDetails["+i+"].sysQty").value;
		//	  var qty=document.getElementById("allocationDetails["+i+"].quantity").value;
		//	  if(parseInt(qty)>parseInt(sysQty)){
		//		  	   alert("第["+ (i+1) +"]行申请数量不能大于库存数量");
		//	       		bool=false;
		//	       		return false;
		//		  }
		//	  
	...
	$.ajax({ type : 'post', url: '${BasePath}/supply/manage/returnDefect/u_returnDefectconfirm.sc', dataType: 'json', data: $('#mainForm').serialize(),
		...
		success: function(resultMsg, textStatus){
		   ...
		   if(result){
				...
				//closewindow();
				//var viewReturnDefectUrl="${BasePath}/supply/manage/returnDefect/toSaveConfirm.sc?mainId="+$("#id").val()+"";
...
function deleteConfrimOrder(){
	if(!confirm("您确定要删除此退残确认单吗？")){	
			return false;
  	}
	viewReturnConfirmFrom.action = "${BasePath}/supply/manage/returnDefect/deleteConfrimOrder.sc";
	viewReturnConfirmFrom.submit();
}	
	...
	for(var i=0;i<cks.length;i++){   
		...
		success : function(resultMsg) {
			if (resultMsg) {
				...
				window.location.href = "${BasePath}/supply/manage/returnDefect/toSaveConfirm.sc?mainId="+mainID+"";
			} else {
				alert("提示：["+resultMsg+"]");
			}
		}
...
function toAddDetail() {
   var supplierCode=$('#supplierCode').val();
	...
	var toAddDetailUrl="${BasePath}/wms/stocksmanager/purchasereturn/toPurchaseReturnSel.sc?supplierCode="+supplierCode+"&supplierName="+supplierName+"&returnDefectConfirmId="+$('#returnDefectConfirmId').val()+"&queryTypeFlag=1&url=supply/purchase/returndefect/common_purchasereturn_sel";
...
function allConfirmDetail(){	
	...
	$.ajax({ type : 'post', url: '${BasePath}/supply/manage/returnDefect/updateAllPurchaseDetailConfirm.sc', dataType: 'json', data: $('#allConfrimForm').serialize(),
...
<div class="container">
	...
	<div class="list_content"> 
		...
		<div class="top clearfix">
			<ul class="tab">
				<li class="curr"><span>退残确认单提交</span> </li>
			</ul>
		</div>
		<!--当前位置end-->
		<div class="modify">
...
<form name="viewReturnConfirmFrom" id="viewReturnConfirmFrom"	action="${BasePath}/supply/manage/returnDefect/toSaveConfirm.sc" method="POST">
...
function doSaveOrConfrim(type) {
	...
	if(upTable.rows.length==1){
     ...
     if(!checkInput()){
		...
		$.ajax({ type : 'post', url: '${BasePath}/supply/manage/returnDefect/toUpdateReturnDefeDetail.sc', dataType: 'json', data: $('#mainFormUp').serialize(),
		...
		success : function(resultMsg,textStatus) {	
			...
			if(result){
				...
				if(type==1)
				{
				  ...
				  window.location.href="${BasePath}/supply/manage/returnDefect/toSaveConfirm.sc?mainId="+$("#returnDefectConfirmId").val()+"";
				  //var updateConfrimUrl = "${BasePath}/supply/manage/returnDefect/toSaveConfirm.sc?mainId="+$("#returnDefectConfirmId").val()+"";	
