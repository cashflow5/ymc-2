...
function exportDate(purchaseId) {
  	if(purchaseId!="") {
  		document.pecharseForm.action='${BasePath}/supply/manage/purchaseDetail/exportDatas.sc?purchaseId='+purchaseId;
		document.pecharseForm.method="post";
		document.pecharseForm.submit();
		document.pecharseForm.action="findFulfillOrder.sc";
  	}
  }
  
function  excelSubmit2() {
	var purchaser = $("#purchaser").val();
	if(purchaser!="") {
		document.pecharseForm.action='${BasePath}/supply/manage/purchaseDetail/exportAllDatas.sc';
		document.pecharseForm.method="post";
		document.pecharseForm.submit();
		document.pecharseForm.action="findFulfillOrder.sc";
  	}else {
...
//添加发货单
function addShipments(purchaseCode){
 document.pecharseForm.action='${BasePath}/supply/purchase/addshipments/to_addshipments.sc?purchaseCode='+purchaseCode;
 document.pecharseForm.method="post";
 document.pecharseForm.submit();
}
</script>
...
<div class="container"> 
	...
	<div class="toolbar">
		<div class="t-content">
			<div class="btn" onclick="excelSubmit2();"> <span class="btn_l" ></span> <b class="ico_btn delivery"></b> <span class="btn_txt">导出Excel</span> <span class="btn_r"></span> </div>
		</div>
	</div>
	<div class="list_content"> 
		...
		<div class="top clearfix">
			<ul class="tab">
				<li class="curr"><span>未完采购跟进</span></li>
			</ul>
		</div>
		<!--当前位置end-->
		...
		<div class="modify"> 
			...
			<div class="add_detail_box">
				<form action="findFulfillOrder.sc" name="pecharseForm" id="pecharseForm" method="post">
					<p>
						<label>采购类型：</label>
			...
			<table class="list_table">
				...
				<tr>
					<td class="td0"> <a href="#" onclick="depose('${item['id']?default('')}')" >关闭</a>&nbsp;|
					...
					<a href="#" onclick="exportDate('${item['id']?default('')}','0')" >导出数据</a> |
					<!--未发货采购单 下周上线-->
					<#if item['status']??&&item['status']==1>
					<a href="#" onclick="addShipments('${item['purchase_code']}')">添加发货单</a> 
					<#else>
					添加发货单 
					</#if>
