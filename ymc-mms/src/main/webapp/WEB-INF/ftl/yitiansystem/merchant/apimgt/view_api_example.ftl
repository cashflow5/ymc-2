...
<div class="container">
	...
	<div class="toolbar">
		<div class="t-content">
			<div class="btn" onclick="javascript:openwindow('${BasePath}/openapimgt/api/example/view.sc?api.id=${api.id?default('')}',　800,　500,　'添加API输出示例数据')">
				<span class="btn_l"></span><b class="ico_btn add"></b><span class="btn_txt">添加API输出示例数据</span><span class="btn_r"></span> </div>
		</div>
	</div>
	<!--工具栏end-->
	<div class="list_content">
		...
		<div class="tbox">
			<table class="list_table" ellspacing="0" cellpadding="0" border="0">
				<thead>
                    <tr>
                    	<th width="10%">示例数据分类</th>
                    	<th width="80%">示例数据</th>
                    	<th width="10%">操作</th>
                    </tr>
				</thead>
				<tbody>
                    <#if api.apiExamples?? && api.apiExamples?size != 0>
						<#list api.apiExamples as apiExample>
							<tr>
								<td>${apiExample.exampleCategory?default('')}</td>
								<td>
									<pre>${apiExample.exampleData?default('')?html}</pre>
								</td>
								<td>
									<a href="javascript:void(0);" onclick="openwindow('${BasePath}/openapimgt/api/example/view.sc?id=${apiExample.id?default('')}&api.id=${api.id?default('')}',　800,　500,　'修改API输出示例数据')">修改</a>
									<a href="javascript:void(0);" onclick="deleteApiInputParam('${apiExample.id}')">删除</a>
								</td>
							</tr>
						</#list>
					<#else>
...
function deleteApiInputParam(id) {
	if (confirm('确定删除该API输出示例数据?')) {
		$.ajax({ type : 'post', url: '${BasePath}/openapimgt/api/example/delete.sc', dataType: 'text', data: 'id=' + id,
