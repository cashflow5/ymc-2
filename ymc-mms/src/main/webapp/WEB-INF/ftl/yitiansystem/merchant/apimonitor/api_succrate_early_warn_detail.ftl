				...
				<table class="list_table2">
					<thead>
						<tr>
							...
							<th style="width:100px;">成功调用次数</th>
							<th style="width:100px;">失败调用次数</th>
							<th style="width:100px;">调用成功率</th>
						</tr>
					</thead>
...
$(function(){
	var arrays = [];
	<#list list as item>
	arrays[arrays.length] = { index: '${item_index + 1}', timeQuantum:'${item.timeQuantum!""}', apiMethod:'${item.apiMethod!'-'}', successCallCount:'${item.successCallCount!""}', failCallCount:'${item.failCallCount!"-"}', rate:'${item.ratio?string("0.##")}%' };
	</#list>
	...
	function pageselectCallback(page_index, jq){
		...
		for(var i = start; i <= end; i++) {
			new_content += "<tr><td style='text-align:center;'>" + arrays[i].index + "</td><td>" + arrays[i].timeQuantum + "</td><td>" + arrays[i].apiMethod + "</td><td>" + arrays[i].successCallCount + "</td><td>" + arrays[i].failCallCount + "</td><td>" + arrays[i].rate + "</td></tr>";
		}
		$("#Searchresult").empty();
