...
<div class="container">
	...
	<div class="list_content">
		...
		<div class="modify">
		<form action="${BasePath}/api/report/to_analyze_detail_api_report.sc" name="queryForm" id="queryForm" method="post"> 
			<input type="hidden" id="apiName" name="apiName" value="${apiName}" />
			...
			<div class="wms-top">
  			  		<label><b>API接口：</b></label>${apiName}
  			  		<label><b>查询日期：</b></label>${startTime}&nbsp;至&nbsp;${endTime}
           			<br />
           			<label><b>AppKey持有者：</b></label>
			...
			<div>
				...
				<#if list??&& list?size != 0>
					...
					<tr>
						...
						<td id="avgFrequency_${flag}_${item_index}">${item.avgFrequency!'0'}</td>
						<td id="maxFrequency_${flag}_${item_index}">${item.maxFrequency!'0'}</td>
						<td id="maxAppkeyNums_${flag}_${item_index}">-</td>
	                </tr>
                    </#list>
                    <#--合计-->
                    <tr>
...
function to_export_api_report() {
	...
	var url = "${BasePath}/api/report/to_analyze_detail_api_report.sc";
