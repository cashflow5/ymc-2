...
<div class="container">
	...
	<div class="toolbar">
		<div class="t-content">
			<div class="btn" onclick="javascript:openwindow('${BasePath}/openapimgt/interceptor/mapper/view.sc?api.id=${api.id?default('')}',　800,　500,　'添加API业务请求拦截器')">
				<span class="btn_l"></span><b class="ico_btn add"></b><span class="btn_txt">添加API业务请求拦截器</span><span class="btn_r"></span> </div>
		</div>
	</div>
	<!--工具栏end-->
	<div class="list_content">
		<div class="top clearfix">
			<ul class="tab">
				...
				<li class="curr">
				  <span><a href="#" class="btn-onselc">API拦截器链</a></span>
				</li>
				<li>
 		...
 		<div class="modify"> 
			...
			<table class="list_table" ellspacing="0" cellpadding="0" border="0">
				...
				<tbody>
                    <#if api.apiInterceptorMappers?? && api.apiInterceptorMappers?size != 0>
						<#list api.apiInterceptorMappers as apiInterceptorMapper>
							<tr>
								<td>${apiInterceptorMapper.apiInterceptor.interceptorClass?default('')}</td>
								<td>${apiInterceptorMapper.orderNo?default('')}</td>
								<td>
									<a href="javascript:void(0);" onclick="openwindow('${BasePath}/openapimgt/interceptor/mapper/view.sc?id=${apiInterceptorMapper.id}&api.id=${api.id?default('')}',　800,　500,　'修改API业务请求拦截器')">修改</a>
...
function deleteApiInterceptorMapper(id) {
	if (confirm('确定删除该API拦截器链?')) {
		$.ajax({ type : 'post', url: '${BasePath}/openapimgt/interceptor/mapper/delete.sc', dataType: 'text', data: 'id=' + id,
