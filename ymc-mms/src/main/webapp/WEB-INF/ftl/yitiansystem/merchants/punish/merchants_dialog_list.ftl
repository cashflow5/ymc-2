						...
						<select id="secondProblemId" name="secondProblemId" class="selecttxt">
							...
							<#if problemList??>
								<#list problemList as item>
									<option <#if item['id']??&&vo.secondProblemId??&&vo.secondProblemId==(item['id'])>selected</#if> value="${item['id']!""}">${item['name']?default('')}</option>
...
function chooseSecondProblemId(){
	...
	if(problemId !=null && problemId !=""){
		$.ajax({
			url: '${BasePath}/yitiansystem/merchants/businessorder/dialogList_SecondProblemList.sc?problemId='+problemId+'&time='+(new Date()).valueOf(),
			async: true,
			dataType: "json",
