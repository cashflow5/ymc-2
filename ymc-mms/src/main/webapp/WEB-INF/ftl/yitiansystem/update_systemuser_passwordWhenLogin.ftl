...
<script type="text/javascript">
	//提交表单
  	function submitForm(){
  //	alert("newPassword=="+$("#newPassword").val());
  	 	//alert("oldPasswrod=="+$("#oldPasswrod").val());
  	if($("#newPassword").val()==$('#oldPasswrod').val()) {
  		ygdg.dialog.alert("新密码不能与原密码相同！");
  		return;
  	}
  		if($("#newPassword").val()==""){
			...
			"rand" : Math.random()
			  ...
			  if(data=='success') {
			    	...
			    	//ygdg.dialog.alert("密码长度至少6位且必须包含数字及字母");
			    		$('#newPasswordErrorTip').html('	<span style="color:red;">密码不能少于6个字符，须同时包含字母与数字。</span>');
			  }
  	...
  	{
  		var data = "newPassword="+$('#newPassword').val()+"&oldPassrod="+$('#oldPassrod').val();
  		$.ajax({
			  type: "POST",
			  async :false,
			  url: "${BasePath}/yitiansystem/systemmgmt/mainmanage/u_updateSystemUserPassword.sc",
			  data: {
			"newPassword" : $('#newPassword').val(),
			"oldPassrod" : $('#oldPassrod').val(),
			"rand" : Math.random()
		},
			  cache: false,
			  success: function(data){
			  if(data=='success') {
			  		...
			  		closewindow();
			    		...
			    		$('#newPasswordErrorTip').html('	<span style="color:red;">密码更新失败。</span>');
			  }
		...
		<div class="modify" id="modify">
			<form action="u_updateSystemUserPassword.sc" method="post"  name="systemUserForm1" id="systemUserForm1" style="margin:0px;padding:0p;">
	            ...
	            <div class="add_detail_box">
	            	...
	            	<input type="hidden" name="oldPasswrod" value="${oldPasswrod?default('')}" id="oldPasswrod" />
	            	<p>
						<span style="color:red;">该账号密码已过期，请输入新密码。</span>
			       	</p>
					<p>
						<span>
	            			<label>新密码：</label>
			       			<input name="newPassword" type="password" class="inputtxt" id="newPassword" maxLength="50"/>
			       			<div class="tip" id="newPasswordErrorTip" ></div>
			       			<div class="tip" id="newPasswordTip" >请输入字母与数字，最短6个字符，最长20个字符。</div>
			       		</span>
			       	</p>
			       	<p>
			       		<span>
