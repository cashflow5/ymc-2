	...
	<div class="container">
		...
		<div class="list_content">
			<div class="top clearfix">
				<ul class="tab">
					<li class='curr' ><span>角色操作日志</span></li>
				</ul>
			</div>
		 	<div class="modify">  
				<form id="queryForm" name="queryForm" action="queryOperateLog.sc" method="post">
					<input type="hidden" name="url" id="url" value="operate"/>
					<div class="add_detail_box">
						<p>
						<label>操作人姓名：</label><input type="text" name="user_name" id="user_name" value="${log.user_name?if_exists}" />
						<label>操作时间：</label>
						...
						<input type="text" id="end_time" name="end_time" value="${log.end_time?if_exists}" />
			      		<label>操作IP地址：</label><input type="text" name="operate_ip" id="operate_ip" value="${log.operate_ip?if_exists}" />
