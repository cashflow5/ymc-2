...
<div class="container">
	...
	<div class="list_content"> 
		...
		<!--当前位置end-->
		<div class="modify">
			<form name="form1" id="form1" action="updatePermission.sc" method="post">
			<input type="hidden" name="userPermissionGroupId" id="userPermissionGroupId" value="${userPermissionGroupId}">					
			<h3>小组信息</h3>
				<tr>
					<th width="100">组名:</th>
					<th><input type="text" name="groupName" id="groupName" maxLength="200" value="${userPermissionGroup.groupName!""}" /></th>
				</tr>
			<h3>订单操作权限</h3>
				<table width="100%" class="list_table2">
					<tbody>
						...
						<tr>
							<td>
								<label>
									<input type="checkbox"  id="source_${sourceKey?split("|")[1]}" name="sources" 
										<#if permissionDataList??>
											<#list permissionDataList as item>
													<#if "${sourceKey!''}"?contains('|'+item.dataName)  >
														checked="checked"
													</#if>
											</#list>
										</#if>
										 onclick="sourceClickEventBind(this,'${sourceKey?split("|")[1]}');" value="${sourceKey?split("|")[1]}"/>
										${sourceKey?split("|")[0]}
									...
									<#list sellerMap?keys as sellerKey>
										<tr style="margin:0px;">
											<td width="21.74%" style="margin:0px;">
												<label>
													<input type="checkbox"  id="seller_${sellerKey?split("|")[1]}" name="seller_name_${sourceKey?split("|")[1]}"
										<#if permissionDataList??>
											<#list permissionDataList as item>
													<#if "${sourceKey!''}"?contains('|'+item.dataName) >
													<#assign sellerSubList=sellerMap[sellerKey] />
													<#list item.dataValue?split(',') as dataValueitem>
								                        <#list sellerSubList as sellerSub>										
														<#if "${sellerSub.no}"==dataValueitem >
															checked="checked"
														</#if>
														</#list>
													</#list>	
									   	 	...
									   	 	onclick="sellerClickEventBind(this,'${sourceKey?split("|")[1]}','${sellerKey?split("|")[1]}')" value="${sellerKey?split("|")[1]!""}"/>
											${sellerKey?split("|")[0]}
												</label>
											</td>
											<td  style="margin:0px;">
												...
												<#list sellerSubList as sellerSub>
													<span><label>
													<input type="checkbox"  name="sellerSub_name_${sellerKey?split("|")[1]}" 
										<#if permissionDataList??>
											<#list permissionDataList as item>
													<#if "${sourceKey!''}"?contains('|'+item.dataName) >
														<#list item.dataValue?split(',') as dataValueitem>																												
								                        <#if dataValueitem=="${sellerSub.no}">
															checked="checked"	
														</#if>
														</#list>
													</#if>
									   	 	...
									   	 	onclick="sellerSubClickEventBind(this,'${sourceKey?split("|")[1]}','${sellerKey?split("|")[1]}')" value="${sellerSub.no!""}"/>
											${sellerSub.name}
													</label>
													</span>
											    </#list>
				...
				<table width="100%" id="commodity" class="list_table2">
					<tbody>
						...
						<tr>
							<td>
								<label>
									<input type="checkbox"  id="cat_${testKey!""}" name="categorys"
										<#if permissionDataList??>
												<#list permissionDataList as item>
														<#if item.dataName == "${testKey!''}" >
															checked="checked"
														</#if>
												</#list>
										</#if>
										 onclick="catClickEventBind(this,'${testKey!""}');" value="${testKey!""}"/>
										${testKey!""}
							...
							<td>
								<#list item as brand >
									<label>
									<input type="checkbox"  name="brand_catName_${testKey!""}" 
										<#if permissionDataList??>
											<#list permissionDataList as item>
													<#if item.dataName == "${testKey!''}" >
														<#if item.dataValue?contains("${brand.brandNo}") >
															checked="checked"
														</#if>
													</#if>
											</#list>
										</#if>
									onclick="brandClickEventBind(this,'${testKey!""}')" value="${brand.brandNo!""}"/>
									${brand.brandName}
									</label>
								</#list>
				...
				<h3>处理小组说明</h3>
				<p>
					<textarea id="remark" name="remark" cols="60" rows="4" >${userPermissionGroup.groupRemark!""}</textarea>
				</p>
				<p >
...
function formSubmit(){
	...
	for(var k = 0 ; k < sourcesArr.length ;k++){
			if(sourcesArr[k].checked){
		      ...
		      $("#source_sellers_val_"+sourcesArr[n].value).val(sellerdata);
		      n++;
			}
    }
	$("#form1").submit();  
