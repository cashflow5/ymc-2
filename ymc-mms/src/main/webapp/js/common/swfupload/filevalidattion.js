...
function FileValidation(ignoreValidation) {
	...
	this.KB = 1024;
	this.fileSizes = [ this.KB * 10, this.KB * 60, this.KB * 10, this.KB * 10, this.KB * 500, this.KB * 500, this.KB * 25, this.KB * 10, this.KB * 10 ];
...
FileValidation.prototype.validate = function (file) {
	...
	for (var i = this.startIndex; defaultMessage != null && i < this.endIndex; i++) {
		if (file.name.match(new RegExp(this.fileNameRegexps[i], 'g')) != null) {
			if (file.size > (this.fileSizes[i] + this.KB)) {
				return '文件大小不能超过：' + SWFUpload.speed.formatBytes(this.fileSizes[i]);
			}
			if (file.loaded && (file.width + '*' + file.height).match(new RegExp(this.filePixelRegexps[i], 'g')) == null) {
