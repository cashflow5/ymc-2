...
KE.configServer = function(url,fileName,no){
	this.siconfig.url = url;
	this.siconfig.fileName = fileName;
	this.siconfig.no = no;
}
...
KE.setting = {
	...
	items : [
		...
		'italic', 'underline', 'strikethrough', 'removeformat', '|','image','serverimage',
...
KE.plugin['serverimage'] = {
	...
	click : function(id) {
		...
		this.dialog = new KE.dialog({
			id : id,
			cmd : 'serverimage',
			file : 'serverimage.html',
			width : 700,
			height : 550,
			loadingMode : true,
			title : KE.lang['serverimage'],
			yesButton : KE.lang['yes'],
			noButton : KE.lang['no']
		});
		this.dialog.show();
	...
	check : function(imgurl) {
		if (!imgurl.match(/\.(jpg|jpeg|gif|bmp|png)(\s|\?|$)/i)) {
			alert(KE.lang['invalidImg']);
			return false;
		}
		return true;
	...
	//	var title = KE.$('imgTitle', dialogDoc).value;
		...
		var doc = dialogDoc || document;
		
		var urls = doc.getElementsByName(KE.siconfig.fileName);
		var url =  null;
		for(var i = 0 ; i < urls.length ; i++){
			url = urls[i];
			if(url.checked){
				if (!this.check(url.value)) continue;
				this.insert(id, url.value, '', 0, 0, 0, '');
			}
		}
