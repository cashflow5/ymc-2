...
function checkPropGroupName(){
	...
	if($("#propGroupName").val() == "" || $("#propGroupName").val() == oldPropGroupName){
		return true;
	}
...
var config={
	form:"propGroupFrom",submit:submitForm,
 	fields:[
		{name:'propGroupName',allownull:false,regExp:"notempty",focusMsg:'请输入属性组名称',rightMsg:'输入格式正确',errorMsg:'属性组名称不能为空',msgTip:'propGroupNameTip'},
...
var arr = new Array();
	...
	var isSpec = null;
	  $(document).ready(function(){
		  isSpec = $('#isSpec').val();
		  path = $("#basepath").val();
		  oldPropGroupName = $('#propGroupName').val();
	...
	function  removeLiT(obj){
		var rowId = $(obj).parent().attr("id");
		var catId = $(obj).attr('role');
		var propGroupId = $('#propGroupId').val();
		if(deleteCatPropGroup(isSpec, propGroupId, catId)){
			$("#"+rowId).remove();
			renameLi();
 ...
 * 删除属性组分类
 */	 
function deleteCatPropGroup(isSpec, propGroupId, catId){
	 var result = false;
	 $.ajax({
       type: "POST",
       async:false,
       url: path+"/yitiansystem/commoditymgmt/commodityinfo/propgroup/d_CatPropGroup.sc",
       data:{
