...
$(document).ready(function(){
	...
	$("#priceScopes>tbody>tr:eq(0)").remove();
	
	
	var rowSizes = $("#priceScopes>tbody>tr").size();
	//alert(rowSizes);
	if(rowSizes>0){
		$("#addRow_div").hide();
	}else{
...
function loadPriceScope(list){
	...
	for(var i = 0; i < list.length; i ++){
		...
		newRow_.find("td:eq(2)>input:eq(0)").val(list[i].maxPrice);
...
function createNewRow(){
	...
	//createedNewRow.find("td:eq(0)>span:eq(0)").text($("#catName").val());
...
function fn_addRows(current){
	...
	//addRow.find("td:eq(0)>span:eq(0)").text($("#catName").val());
...
function registNotNull(){
 	//alert(10);
 	var result = false;
	$("#tableSort>input:[type='text']:not([name='remark'])").each(function(i, obj){
		if(obj.value==""){
			$(obj).addClass('store-input-err');
			result = true;
		} else {
			$(obj).removeClass('store-input-err');
 		}
 	});
 	return result;
}
...
function saveBeforeValidateNull(){
	var result = false;
  	// 验证是不是数字....
  	$("#tableSort>input:[type='text']:not(:[name='remark'])").each(function(){
...
function checkMaxPrice(){
	//var resultObj = {max_price:,price_list:};
	var result = true;
	var trs = $("#priceScopes>tbody");
	var number_list = new Array();
	var number_max = 0;
	for(var i = 0; i < trs.find("tr").size(); i ++){
		var currentRow = trs.find("tr:eq("+i+")");
		if(currentRow.find("td:eq(2)>input:eq(0)").val()!=defaultPriceValue){
			for(var a = 1; a < 3; a++){
				var current_number = currentRow.find("td:eq("+a+")>input:eq(0)").val();
				number_list.push(parseInt(current_number));
			}
		}else{
			number_max = currentRow.find("td:eq(1)>input:eq(0)").val();
		}
	}
	number_list.sort();
	
	if(number_list[0]>number_max){
		return false;// 表示最大值设置错误
	}
	return true; // 表示最大值设置正确
}
*/
/**
 * 检测是否存在有"---" 以上的字样..
...
function savePriceScopes(){
	...
	var catId = $("#catId").val();
	var catStructName = $("#catStructName").val();
	...
	for(var i = 0; i < trs.find("tr").length; i ++){
		...
		var priceScopeId = $("#priceScopeId").val();
	...
	$.ajax({
		...
		success:function(d){
			...
			if(o.result==1){
				alert("保存成功!");
				if(priceScopeId!==""){
					fn_toEditPriceScopeByEntry(catId,catName,catStructName,priceScopeId);
				}else{
