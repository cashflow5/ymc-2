...
function doExportForDay() {
	...
	$.ajax({
		...
		success : function(resultMsg) {
			if (resultMsg.success) {
				$("#doExportForDayBtn").removeAttr('onclick');
				
				doWMSExport("queryForm", doExportForDayUrl);
			} else {
				alert(resultMsg.msg);
			}
		}
...
// 导出
function doExportForAllDayDetail() {
	if (document.getElementById("bTime").value == ''
			|| document.getElementById("eTime").value == '') {
		alert('请选择时间范围！');
		return;
	...
	$.ajax({
		...
		success : function(resultMsg) {
			if (resultMsg.success) {
				$("#doExportForAllDayDetailBtn").removeAttr('onclick');
				
				doWMSExport("queryForm", doExportForAllDayDetailUrl);
			} else {
				alert(resultMsg.msg);
			}
		}
