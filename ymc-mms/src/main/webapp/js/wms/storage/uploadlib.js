...
FileUpload.prototype = {
    ...
    initialize: function(form, folder, options){
        ...
        this.Form = aa(form);//表单
        this.Folder = aa(folder);//文件控件存放空间
        this.Files = [];//文件集合
    ...
    SetOptions: function(options){
        this.options = {//默认值
            ...
            ExtIn: ["gif", "jpg","png","swf","doc","zip","rar","iso", "htm","html","txt"],//允许后缀名
            onNotExtIn: function(){
            },//不是允许后缀名时执行
...
function AddList(rows){
	//根据数组来添加列表
	var FileList = aa("idFileList"), oFragment = document.createDocumentFragment();
	//用文档碎片保存列表
	Each(rows, function(cells){
		var row = document.createElement("tr");
		Each(cells, function(o,index,len){
			var cell = document.createElement("td");
			cell.className = index == len ? "td0" : "";
