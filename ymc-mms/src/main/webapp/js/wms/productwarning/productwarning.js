function validatorProduct(checkBoxFlag) {
   ...
   if(checkBoxFlag=="1"){
	...
	var unitPriceRs = validatorInputFile('unitPrice',/^([0-9]{1,5}|0)(\.\d{2})?$/,mesg,"unitPriceTip",rowIndexArray,checkBoxFlag);
	validatorResult = unitPriceRs ? validatorResult : unitPriceRs;
	
	var mesg = "数量格式不正确，必须是非零的正整数且位数最大7位。";
	var quantityRs = validatorInputFile('quantity',/^([0-9]{1,7}|0)(\d{0})?$/,mesg,"quantityTip",rowIndexArray,checkBoxFlag);
	validatorResult = quantityRs ? validatorResult : quantityRs;
	
	var mesg = "扣点比率格式不正确，必须是数字。(如：0.001)";
	var deductionRateRs = validatorInputFile('deductionRate',/^(([1-9]\d*)|0)(\.\d{3})?$/,mesg,"deductionRateTip",rowIndexArray,checkBoxFlag);
	validatorResult = deductionRateRs ? validatorResult : deductionRateRs;
	
	var mesg = "税率格式不正确，必须是数字。(如：0.001)";
	var rateRs = validatorInputFile('rate',/^(([1-9]\d*)|0)(\.\d{3})?$/,mesg,"rateTip",rowIndexArray,checkBoxFlag);
	validatorResult = rateRs ? validatorResult : rateRs;
