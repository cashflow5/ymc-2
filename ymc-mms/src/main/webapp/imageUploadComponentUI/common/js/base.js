...
function priceCurrencyFormat(price) {
	price = setScale(price, belle.priceScale, belle.priceRoundType);
	return belle.currencySign + price;
}

// 格式化商品价格货币（包含货币单位）
function priceUnitCurrencyFormat(price) {
	price = setScale(price, belle.priceScale, belle.priceRoundType);
	return belle.currencySign + price + belle.currencyUnit;
}

// 格式化订单金额货币
function orderCurrencyFormat(price) {
	price = setScale(price, belle.orderScale, belle.orderRoundType);
	return belle.currencySign + price;
}
...
$().ready( function() {
	...
	*/
	...
	$.message = function () {
		...
		/*$('#messageWindow .windowTop').dialog({content: messageText});*/
		$.dialog({content: messageText});
		$messageButton.focus();
	}
